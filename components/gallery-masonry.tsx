    ...
    <>
      <div className="columns-1 gap-5 sm:columns-2 xl:columns-3">
        {items.map((item, index) => (
          <motion.button
            className="group relative mb-5 block w-full break-inside-avoid overflow-hidden rounded-[1.9rem] border border-line bg-white text-left shadow-[0_20px_55px_rgba(56,39,27,0.08)]"
            key={item.id}
      ...
      <AnimatePresence>
        {activeItem ? (
          <motion.div
            animate={{ opacity: 1 }}
            className="fixed inset-0 z-[80] bg-black/82 px-4 py-5 backdrop-blur-md"
            exit={{ opacity: 0 }}
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              ...
              <div className="relative flex-1 overflow-hidden rounded-[2rem] border border-white/10 bg-black/40 p-3">
                <div className="flex h-full items-center justify-center overflow-hidden rounded-[1.5rem] bg-black/60">
                  {activeItem.kind === "image" ? (
                    ...
                    <video
                      className="max-h-[78vh] w-full rounded-[1.25rem] bg-black object-contain"
                      controls
                      playsInline
                      preload="metadata"
                      src={activeItem.src}
                ...
                <button
                  className="absolute left-6 top-1/2 inline-flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-black/50 text-white backdrop-blur-sm"
                  onClick={() =>
                    setActiveIndex((current) =>
                      current === null ? current : (current - 1 + items.length) % items.length,
                    )
                ...
                <button
                  className="absolute right-6 top-1/2 inline-flex h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-black/50 text-white backdrop-blur-sm"
                  onClick={() =>
                    setActiveIndex((current) =>
                      current === null ? current : (current + 1) % items.length,
                    )
              ...
              >
                <p className="text-base leading-8 text-white/82">{activeItem.caption}</p>
                {activeItem.kind === "video" && activeItem.durationLabel ? (
                  <p className="mt-2 text-xs font-semibold uppercase tracking-[0.24em] text-[#f2c978]">
